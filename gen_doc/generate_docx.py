...
def getdocx(info_path,CHAT_ID):
    ...
    if pl_num == 1:
        if pl_type[0] == 0:
            ...
            add_mixed_style_text(plaintiff_paragraph, \
                [(f"原告：", True), \
                (f"{pl_name[0]}, {pl_gender[0]}, {pl_race[0]}族，{pl_birth[0]}出生，住{pl_home[0]}，身份证号码：{pl_id[0]}，联系电话：{pl_phone[0]}。", False)])
        else:
            plaintiff_paragraph = doc.add_paragraph()
            add_mixed_style_text(plaintiff_paragraph, \
                [(f"原告：", True), \
        ...
        for i in range(pl_num):
            if pl_type[i] == 0:
                ...
                add_mixed_style_text(plaintiff_paragraph, \
                    [(f"原告{i+1}：", True), \
                    (f"{pl_name[i]}, {pl_gender[i]}, {pl_race[i]}族，{pl_birth[i]}出生，住{pl_home[i]}，身份证号码：{pl_id[i]}，联系电话：{pl_phone[i]}。", False)])
            else:
                plaintiff_paragraph = doc.add_paragraph()
                add_mixed_style_text(plaintiff_paragraph, \
                    [(f"原告{i+1}：", True), \
    ...
    if de_num == 1:
        if de_type[0] == 0:
            ...
            add_mixed_style_text(defendant_paragraph, \
                [("被告：", True), \
                (f"{de_name[0]}, {de_gender[0]}, {de_race[0]}族，{de_birth[0]}出生，住{de_home[0]}，身份证号码：{de_id[0]}，联系电话：{de_phone[0]}。", False)])
        else:
            defendant_paragraph = doc.add_paragraph()
            add_mixed_style_text(defendant_paragraph, \
                [(f"被告：", True), \
        ...
        for i in range(de_num):
            if de_type[i] == 0:
                ...
                add_mixed_style_text(defendant_paragraph, \
                    [(f"被告{i+1}：", True), \
                    (f"{de_name[i]}, {de_gender[i]}, {de_race[i]}族，{de_birth[i]}出生，住{de_home[i]}，身份证号码：{de_id[i]}，联系电话：{de_phone[i]}。", False)])
            else:
                defendant_paragraph = doc.add_paragraph()
                add_mixed_style_text(defendant_paragraph, \
                    [(f"被告{i+1}：", True), \
    ...
    doc.save(f'/root/LawSparkServer_v7/output/起诉状_{CHAT_ID}.docx')
