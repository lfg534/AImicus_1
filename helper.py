...
# def save_data(path: str, file_name: str, history: list, paras: dict, contexts: dict, message_stage: int, case_type : int ,**kwargs):
#     if not os.path.exists(path):
#         os.makedirs(path)
#     with open(f"./{path}/{file_name}.json", 'w', encoding='utf-8') as f:
        # json.dump({"history": history, "paras": paras, "contexts": contexts, "message_stage":message_stage, "case_type" : case_type ,**kwargs}, f)

def save_data(path: str, file_name: str, history: list, message_stage: int, case_type : int , is_final : int, person_data: str, person_count: int ,**kwargs):
    print('正在print')
    if not os.path.exists(path):
        os.makedirs(path)
    with open(f"./{path}/{file_name}.json", 'w', encoding='utf-8') as f:
