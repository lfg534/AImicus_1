...
//随着实时输入,每读出一个单词,调用一次下面的回调函数

recognition.onresult = function (event) {
    let voiceTranscript = '';//随着输入
    let flag = 'interim'

    //event.resultIndex代表当前句子的index,即当前这个单词所在的句子的index
    //event.results.length代表一次语音(从开始到结束)识别到的句子的个数
    //event.results -> SpeechRecognitionResultList
    //event.results[0] -> SpeechRecognitionResult; 一次识别的话
    //event.results[0][0] -> SpeechRecognitionAlternative;  从句子开头到这次识别的单词
    for (let i = event.resultIndex; i < event.results.length; i++) {
        let transcript = event.results[i][0].transcript; //从句子开头到这次识别的单词
        console.log('transcript: ' + transcript)
        if (event.results[i].isFinal) {
            voiceTranscriptFinal += transcript;//一句话完成累加入voiceTranscriptFinal
            flag = 'final'
        } else {
            voiceTranscript += transcript; //把单词累加成一句话
        }
    }
